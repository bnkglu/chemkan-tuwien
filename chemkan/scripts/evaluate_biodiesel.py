...
_METRIC_CONVENTION = ("normalized trajectory MSE (Eq. 18): mean over modeled state variables "
                      "(species-only, biodiesel Stage-1), summed over observation times, then "
                      "mean over trajectories, using train-only min-max normalization "
...
def save_prediction_artifact(run_dir: Path, res: dict, split: str, ckpt_path: Path,
    ...
    return save_predictions(
        ...
        t=res["t"].numpy(),
        initial_conditions=res["truth"][0].numpy(),   # Y0 (B, m)
        species=res["species"],
